...
const bn_mul = new BN(cmultiplier); // multiplier for coins in accounts
    ...
    const bn_2000eth = new BN(2000).times(bn_1eth);
    const bn_1000eth = new BN(1000).times(bn_1eth);
    const bn_100eth = new BN(100).times(bn_1eth);
    const bn_10eth = new BN(10).times(bn_1eth);
    ...
    const bn_0_05eth = new BN(0.05).times(bn_1eth);
    ...
    const bn_0_5 = new BN(0.5);
    const bn_0_4 = new BN(0.4);
    const bn_0_3 = new BN(0.3);
    const bn_0_2 = new BN(0.2);
...
fileStream.pipe(evts.through(function (data) {
    ...
    //DataModel.save(function (err, dataModel) {
        ...
        for (var i = 0; i < lines.length ; i++) {
            ...
            if (bal.isGreaterThan(bn_2000eth)) {
                console.error(`${nTotal} : ${bal} is > ${bn_maxeth} , skip this line`);
                continue;
            }
            if (bal.isLessThan(bn_0_05eth)) {
            ...
            var newbal = new BN(0);
            var rembal = new BN(0);
            if (bal.isLessThanOrEqualTo(bn_10eth))
            {
                newbal = new BN(bal.multipliedBy(bn_0_5)).integerValue();

            } else if (bal.isLessThanOrEqualTo(bn_100eth)) {
                newbal = new BN(5.0).times(bn_1eth);
                rembal = bal.minus(bn_10eth);
            //console.error(`\t"balance" : ${bal.toString(16,40)}, ${rembal.div(bn_1eth).toString(10,40)}`);
                newbal = newbal.plus(rembal.multipliedBy(bn_0_4)).integerValue();
            //console.error(`\t"newbalance" : ${newbal.toString(16,40)}`);

            } else if (bal.isLessThanOrEqualTo(bn_1000eth)) {
                newbal = new BN(41.0).times(bn_1eth);
                rembal = bal.minus(bn_100eth);
                newbal = newbal.plus(rembal.multipliedBy(bn_0_3)).integerValue();
            //console.error(`\t"balance" : ${bal.toString(16,40)}, ${rembal.div(bn_1eth).toString(10,40)}`);
            //console.error(`\t"newbalance" : ${newbal.toString(16,40)}`);
    
            } else if (bal.isLessThanOrEqualTo(bn_2000eth)) {
                newbal = new BN(311.0).times(bn_1eth);
                rembal = bal.minus(bn_1000eth);
                newbal = newbal.plus(rembal.multipliedBy(bn_0_2)).integerValue();
            //console.error(`\t"balance" : ${bal.toString(16,40)}, ${rembal.div(bn_1eth).toString(10,40)}`);
            //console.error(`\t"newbalance" : ${newbal.toString(16,40)}`);
            }
